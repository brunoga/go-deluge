...
var torrent = flag.String("torrent", "", "torrent to add (url or file path)")
var addPaused = flag.String("add_paused", false, "if true, torrent is added in the paused state")

func main() {
	flag.Parse()

	if *delugeAddress == "" || *delugePassword == "" || *torrent == "" {
		fmt.Printf("-deluge_address, -deluge_password and -torrent must all be provided and non-empty")
		return
	}
